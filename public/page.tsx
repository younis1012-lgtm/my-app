...
const STORAGE_KEY = 'yk-quality-stage2-v1';
...
const checklistTemplates: Record<ChecklistTemplateKey, { label: string; title: string; category: string; items: { description: string; responsible: string }[] }> = {
  general: {
    label: 'רשימה כללית',
    title: 'רשימת תיוג',
    category: 'עבודות עפר',
...
export default function Page() {
  ...
  const deleteProject = (projectId: number) => {
    ...
    setProjects(
      nextProjects.map((item, index) => ({ ...item, isActive: index === 0 && nextCurrentProjectId === item.id }))
    );
  ...
  return (
    <div style={pageStyle}>
      ...
      <div style={navRowStyle}>
        ...
        <NavButton label="אי התאמות" active={section === 'nonconformances'} onClick={() => setSection('nonconformances')} />
      ...
      <div style={layoutStyle}>
        <main style={mainCardStyle}>
          {section === 'home' && (
            <div>
              ...
              <div style={heroBoxStyle}>
                <div style={{ fontWeight: 800, fontSize: 22, marginBottom: 8 }}>ברוך הבא למערכת</div>
                <div style={{ color: '#475569', lineHeight: 1.7 }}>
                  בחר פרויקט, ולאחר מכן עבור למסך הרלוונטי. תחת רשימות תיוג תוכל לבחור טופס ייעודי: עבודות מעקות, פיזור מצעים, אבני שפה או הידוק רגיל.
                </div>
              </div>
            </div>
          )}
          ...
          {section === 'nonconformances' && (
            <div>
              <h2 style={sectionTitleStyle}>אי התאמות</h2>
              {guardedBody || (
                <>
                  <div style={formGridStyle}>
                    <Field label="כותרת">
                    ...
                    <Field label="תיאור אי התאמה" full>
                  ...
                  <div style={buttonRowStyle}>
                    <button style={primaryButtonStyle} onClick={saveNonconformance}>שמור אי התאמה</button>
          ...
          {section === 'preliminary' && (
            <div>
              ...
              {guardedBody || (
                <>
                  <div style={navRowStyle}>
                    <NavButton label="ספקים" active={preliminaryTab === 'suppliers'} onClick={() => setPreliminaryTab('suppliers')} compact />
                    <NavButton label="קבלני משנה" active={preliminaryTab === 'subcontractors'} onClick={() => setPreliminaryTab('subcontractors')} compact />
                    <NavButton label="חומרים" active={preliminaryTab === 'materials'} onClick={() => setPreliminaryTab('materials')} compact />
                  </div>
                  ...
                  {preliminaryTab === 'subcontractors' && (
                    <>
                      ...
                      <div style={buttonRowStyle}>
                        <button style={primaryButtonStyle} onClick={() => savePreliminary('subcontractors')}>שמור קבלן משנה</button>
                      </div>
                    </>
                  )}
        ...
        <aside style={sideCardStyle}>
          ...
          <div style={smallSectionTitleStyle}>אי התאמות</div>
          {projectNonconformances.length === 0 ? (
            <div style={emptyBoxStyle}>אין אי התאמות שמורות.</div>
          ) : (
...
function labelForPreliminary(subtype: PreliminaryTab) {
  ...
  if (subtype === 'subcontractors') return 'קבלני משנה';
  return 'חומרים';
}
