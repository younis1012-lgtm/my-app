...
test('legacy QA viewer sends NCR from the plain system address with attribution and reply-to',async()=>{const s=setup({authUser:{id:'user',email:'legacy-user@users.yk-quality.invalid',app_metadata:{legacy_username:'ה״א'},user_metadata:{name:'ה״א'}},accessUsers:[{username:'ה״א',display_name:'עדנאן שבת',project_ids:['project']}],projectEmailRows:[{id:'one',email:'shbat.adnan.1991@gmail.com',name:'עדנאן שבת',role:'ה״א',smtp_app_password:''},{id:'system',email:'sender@example.com',name:'יונס אבראהים',role:'בקר איכות',smtp_app_password:'SERVER_SECRET'}],role:'readonly'});const response=await s.directory('projectId=project&module=nonconformances');const data=await response.json();assert.equal(data.operator.email,'shbat.adnan.1991@gmail.com');assert.equal(data.operator.name,'עדנאן שבת');assert.equal(data.senders.find(x=>x.email==='sender@example.com').name,'sender@example.com');await s.send({module:'nonconformances',senderEmail:'sender@example.com'});assert.equal(s.message.replyTo,'shbat.adnan.1991@gmail.com');assert.equal(s.message.from,'sender@example.com');assert.match(s.message.text,/נשלחה על ידי הבטחת איכות — עדנאן שבת/);assert.match(s.message.html,/נשלחה על ידי הבטחת איכות — עדנאן שבת/);assert.match([...s.history.values()][0].body,/נשלחה על ידי הבטחת איכות — עדנאן שבת/);});
