      ...
      <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
        <button
          ...
          onClick={() =>
            set({
              signerName: value.signerName || defaultSignerName,
              signature:
                value.signature || value.signerName || defaultSignerName,
        ...
        >
          ...
          <input
            ...
            style={{ display: "none" }}
            onChange={(event) => {
              uploadSignatureToThisForm(event.target.files?.[0]);
              event.currentTarget.value = "";
            }}
          />
        </label>
...
export default function Page() {
  ...
  const saveChecklist = async () => {
    ...
    const unsignedItems = checklistForm.items.filter(
      (item: any) =>
        !item.signature?.signerName ||
        !item.signature?.signature ||
        !item.signature?.signedAt,
    );
    if (unsignedItems.length)
      return alert(
        "יש להשלים חתימה דיגיטלית לכל סעיף בקרה לפני שמירת רשימת התיוג.",
      );
    ...
    await withSaving(async () => {
      if (cloudEnabled) {
        ...
        setSavedChecklists((prev) =>
          editingChecklistId
            ? prev.map((item) =>
                item.id === editingChecklistId ? record : item,
              )
            : [record, ...prev],
        );
      }
