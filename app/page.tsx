...
const readProjectEmailUsers = (): ProjectEmailUser[] => {
  ...
  try {
    const parsed = JSON.parse(
      window.localStorage.getItem(PROJECT_EMAIL_USERS_STORAGE_KEY) || "[]",
    );
...
const PROJECT_EMAIL_USERS_FACTOR_LABEL = "__PROJECT_EMAIL_USERS__";

const projectUsersFromLegend = (
  projectId: string,
  legend?: ProjectLegend | null,
): ProjectEmailUser[] => {
  const factor = legend?.extraFactors?.find(
    (item) => item.label === PROJECT_EMAIL_USERS_FACTOR_LABEL,
  );
  if (!factor?.value) return [];
  try {
    const parsed = JSON.parse(factor.value);
    if (!Array.isArray(parsed)) return [];
    return parsed
      .map((item: any) => ({
        id: String(item?.id || crypto.randomUUID()),
      ...
      .filter((item: ProjectEmailUser) => item.email);
  } catch {
    return [];
  }
};

const mergeProjectUsersIntoLegend = (
  legend: ProjectLegend,
  users: ProjectEmailUser[],
): ProjectLegend => ({
  ...legend,
  extraFactors: [
    ...(legend.extraFactors ?? []).filter(
      (item) => item.label !== PROJECT_EMAIL_USERS_FACTOR_LABEL,
    ),
    {
      id: PROJECT_EMAIL_USERS_FACTOR_LABEL,
      label: PROJECT_EMAIL_USERS_FACTOR_LABEL,
        ...
        (item: any, index) =>
          ({
            ...
            signature: item?.signature
              ? {
                  role: String(
                    item.signature?.role ?? item?.responsible ?? "גורם אחראי",
                  ),
                  signerName: String(
                    item.signature?.signerName ?? item?.inspector ?? "",
                  ),
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        <button type="button" style={styles.secondaryBtn} onClick={save}>
          שמור חתימה
        </button>
        <button type="button" style={styles.secondaryBtn} onClick={clear}>
          נקה
        </button>
          ...
          <DigitalSignaturePad
            onSave={(signatureDataUrl) => {
              set({
                ...
                signedAt:
                  value.signedAt || new Date().toISOString().slice(0, 10),
              });
...
function ChecklistsSection({
  ...
  >;
  const [digitalSignatureItemId, setDigitalSignatureItemId] = useState<
    string | null
  >(null);
  const isRoad806Checklist =
    isRoad806Value(projectName) ||
    isRoad806Value(checklistForm.projectNameDisplay) ||
    isRoad806Value(checklistForm.projectName) ||
    isRoad806Value(checklistForm.location);
        ...
        >
          ...
          >
            <label>
              ...
              <input
                value={
                  (checklistForm as any).projectNameDisplay || projectName || ""
                }
                onChange={(event) =>
                  setField("projectNameDisplay", event.target.value)
                }
                style={inputStyle}
              />
            </label>
            ...
            <label>
              ...
              <input
                ...
                onChange={(event) =>
                  setField("roadStructure", event.target.value)
                }
                style={inputStyle}
              />
            </label>
            <label>
              ...
              <input
                ...
                onChange={(event) =>
                  setField("checklistNo", event.target.value)
                }
                style={inputStyle}
              />
            </label>
            <label>
              ...
              <input
                ...
                onChange={(event) =>
                  setField("stationSection", event.target.value)
                }
                style={inputStyle}
              />
            </label>
            <label>
              ...
              <input
                ...
                onChange={(event) =>
                  setField("toStationSection", event.target.value)
                }
                style={inputStyle}
              />
            </label>
        ...
        >
          <div>
            ...
            <div style={{ color: "#64748b", marginTop: 4 }}>
              ...
              חתימה, תאריך ותעודת מעבדה / הערות. ניתן לשמור, לעדכן, לצרף מסמך
              מול מודד ולצרף מסמכי בדיקה/מעבדה לפי תיאור התהליך.
            </div>
          </div>
          <button
            type="button"
                            ...
                            onChange={(event) =>
                              updateItemSignature(item.id, {
                                ...
                                signerName:
                                  ...
                                  (isRoad806Checklist &&
                                  isSurveyorRole(item.responsible)
  ...
  return (
    ...
    >
      ...
      >
        ...
        {legend.extraFactors
          .filter((factor) => factor.label !== PROJECT_EMAIL_USERS_FACTOR_LABEL)
          .map((factor) => (
            <div
              key={factor.id}
              style={{
...
function getApprovalDisplayStatus(record: any) {
  const status = String(
    record?.status ?? record?.approval?.status ?? "",
  ).trim();
  return status === "מאושר" ? "מאושר" : "בטיפול";
}

function formatExpiredStatus(expiryDate: unknown) {
  const value = String(expiryDate ?? "").trim();
  if (!value) return "בטיפול";
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const expiry = new Date(value);
  if (!Number.isNaN(expiry.getTime()) && expiry < today) return "❌ פג תוקף";
  return "מאושר";
}

function getPrimaryRequiredDocument(record: any) {
  const candidates = [
    ...
    ...(Array.isArray(record?.licenses) ? record.licenses : []),
    ...(Array.isArray(record?.certificates) ? record.certificates : []),
    ...(Array.isArray(record?.supplier?.requiredDocuments)
      ? record.supplier.requiredDocuments
      : []),
    ...(Array.isArray(record?.subcontractor?.requiredDocuments)
      ? record.subcontractor.requiredDocuments
      : []),
    ...(Array.isArray(record?.material?.requiredDocuments)
      ? record.material.requiredDocuments
      : []),
  ];
  return (
    candidates.find(
...
function getDocumentApprovalDate(record: any) {
  const doc: any = getPrimaryRequiredDocument(record);
  return String(
    doc?.approvalDate ??
      doc?.approval_date ??
      doc?.attachedAt ??
...
function getDocumentExpiryDate(record: any) {
  const doc: any = getPrimaryRequiredDocument(record);
  return String(
    doc?.expiryDate ??
      doc?.expiry_date ??
...
function getChecklistLocation(record: any) {
  return (
    ...
    record?.structure ||
    record?.location ||
    record?.items?.find?.((item: any) => item?.element || item?.location)
      ?.element ||
    record?.items?.find?.((item: any) => item?.element || item?.location)
      ?.location ||
    ""
...
function getPreliminaryName(record: any) {
  if (record?.subtype === "suppliers")
    return record?.supplier?.supplierName || record?.supplierName || "";
...
function getPreliminaryMainField(record: any) {
  if (record?.subtype === "suppliers")
    return record?.supplier?.suppliedMaterial || "";
  if (record?.subtype === "subcontractors")
    return record?.subcontractor?.field || "";
  if (record?.subtype === "materials")
    return record?.material?.materialName || record?.material?.usage || "";
  return "";
}

function getPreliminarySecondaryField(record: any) {
  if (record?.subtype === "materials")
    return record?.material?.materialName || record?.material?.usage || "";
  return getDocumentApprovalDate(record);
}
...
type ProjectUsersSectionProps = {
  ...
  users: ProjectEmailUser[];
  onAddUser: (
    user: Omit<ProjectEmailUser, "id" | "projectId" | "createdAt">,
  ) => void;
        ...
        <>
          <div style={styles.sectionHeader}>
            <div>
              ...
              <p
                style={{ margin: "6px 0 0", color: "#64748b", lineHeight: 1.6 }}
              >
                רשימה זו שייכת לפרויקט {projectName}. בעת שליחת מייל מהפרויקט
                ניתן לבחור מתוכה נמען אחד או כמה נמענים.
              </p>
            </div>
            <button
              type="button"
          ...
          >
            ...
            <input
              ...
              style={inputStyle}
            />
            <button type="button" onClick={add} style={styles.primaryBtn}>
              הוסף משתמש
            </button>
          </div>
          <div
                    ...
                    <tr key={user.id}>
                      ...
                      >
                        <input
                          value={user.role}
                          onChange={(e) =>
                            onUpdateUser(user.id, { role: e.target.value })
                          }
                          style={inputStyle}
                        />
                      </td>
                      ...
                      >
                        <input
                          value={user.company}
                          onChange={(e) =>
                            onUpdateUser(user.id, { company: e.target.value })
                          }
                          style={inputStyle}
                        />
                      </td>
                      ...
                      >
                        <input
                          value={user.phone || ""}
                          onChange={(e) =>
                            onUpdateUser(user.id, { phone: e.target.value })
                          }
                          style={inputStyle}
                        />
                      </td>
...
export default function Page() {
  ...
  const [selectedChecklistTemplateKey, setSelectedChecklistTemplateKey] =
    useState<ChecklistTemplateKey>(() =>
      normalizeChecklistTemplateKey(undefined),
    );
  ...
  const savedSignatureForSigner = (signerName: string, role?: string) => {
    if (
      ...
      (isSurveyorRole(role) || isSurveyorRole(signerName))
    ) {
  ...
  const [projectEmailUsers, setProjectEmailUsers] = useState<
    ProjectEmailUser[]
  >(() => readProjectEmailUsers());
  const [supervisionReports, setSupervisionReports] = useState<
    Array<{
      id: string;
      projectId: string;
      name: string;
  ...
  >(() => {
    ...
    try {
      return JSON.parse(
        window.localStorage.getItem(`${STORAGE_KEY}-supervision-reports`) ||
          "[]",
      );
    } catch {
      return [];
  ...
  const currentSupervisionReports = useMemo(
    () =>
      supervisionReports.filter(
        (item) =>
          normalizeStoredProjectId(item.projectId) ===
          normalizeStoredProjectId(currentProject?.id),
      ),
    [supervisionReports, currentProject],
  );
  const saveSupervisionReports = (next: typeof supervisionReports) => {
    setSupervisionReports(next);
    try {
      window.localStorage.setItem(
        `${STORAGE_KEY}-supervision-reports`,
        JSON.stringify(next),
      );
    } catch {}
  };
  const addSupervisionReport = () => {
    if (!currentProject) return alert("יש לבחור פרויקט");
    const name =
      window.prompt(
        "שם דוח פיקוח עליון",
        `דוח פיקוח עליון ${currentSupervisionReports.length + 1}`,
      ) || "";
    if (!name.trim()) return;
    const status = (
      window.prompt("סטטוס: מאושר או בטיפול", "בטיפול") === "מאושר"
        ? "מאושר"
        : "בטיפול"
    ) as "מאושר" | "בטיפול";
    saveSupervisionReports([
      {
        id: crypto.randomUUID(),
        projectId: normalizeStoredProjectId(currentProject.id),
        name: name.trim(),
        status,
        createdAt: nowLocal(),
      },
  ...
  const updateSupervisionStatus = (id: string) => {
    saveSupervisionReports(
      supervisionReports.map((item) =>
        item.id === id
          ? { ...item, status: item.status === "מאושר" ? "בטיפול" : "מאושר" }
          : item,
      ),
    );
  };
  const deleteSupervisionReport = (id: string) => {
    if (!window.confirm("למחוק דוח פיקוח עליון?")) return;
    saveSupervisionReports(supervisionReports.filter((item) => item.id !== id));
  };

  const saveProjectEmailUsers = (
    updater: (prev: ProjectEmailUser[]) => ProjectEmailUser[],
  ) => {
  ...
  useEffect(() => {
    ...
    const cloudUsers = projectUsersFromLegend(
      projectId,
      projectLegends[projectId] ?? projectLegends[currentProject.id],
    );
    if (!cloudUsers.length) return;
    setProjectEmailUsers((prev) => {
      const withoutProject = prev.filter(
        (user) => normalizeStoredProjectId(user.projectId) !== projectId,
      );
      const next = [...withoutProject, ...cloudUsers];
      writeProjectEmailUsers(next);
      return next;
    });
  }, [currentProject?.id, projectLegends]);

  const addProjectEmailUser = (
    user: Omit<ProjectEmailUser, "id" | "projectId" | "createdAt">,
  ) => {
    if (!currentProject) return alert("יש לבחור פרויקט");
  ...
  const updateProjectEmailUser = (
    id: string,
    patch: Partial<ProjectEmailUser>,
  ) => {
    saveProjectEmailUsers((prev) =>
      prev.map((user) =>
        user.id === id
  ...
  const saveCurrentProjectEmailUsers = async () => {
    if (!currentProject) return alert("יש לבחור פרויקט");
    const projectId = normalizeStoredProjectId(currentProject.id);
    const normalizedUsers = projectEmailUsers
      .filter((user) => normalizeStoredProjectId(user.projectId) === projectId)
      .map((user) => ({
        ...user,
        projectId,
    ...
    writeProjectEmailUsers(localUsers);
    setProjectEmailUsers(localUsers);

    const baseLegend = normalizeProjectLegend(
      projectLegends[projectId] ??
        projectLegends[currentProject.id] ??
        currentProjectLegend,
      currentProject.name,
    );
    const nextLegend = mergeProjectUsersIntoLegend(baseLegend, normalizedUsers);
    ...
    try {
      ...
      alert("משתמשי הפרויקט נשמרו בענן ובהצלחה");
    } catch (error) {
      console.error("Failed to save project email users", error);
      alert(`המשתמשים נשמרו מקומית, אך שמירת הענן נכשלה: ${errorText(error)}`);
    }
  };
  ...
  const selectedChecklistLabel = checklistTemplateLabel(
    selectedChecklistTemplateKey,
  );
  ...
  const loadChecklist = (record: ChecklistRecord) => {
    ...
    setSelectedChecklistTemplateKey(
      normalizeChecklistTemplateKey(record.templateKey),
    );
  ...
  const embeddedAttachmentForExport = (
    ...
    const isPdf =
      type.includes("pdf") || src.startsWith("data:application/pdf");
    ...
    const embedded = rows
      .map(({ item, attachment }) =>
        embeddedAttachmentForExport(
          attachment,
          `${checklistAttachmentLabel(attachment.kind)} - ${String(item.description ?? "")}`,
        ),
      )
      .join("");
  ...
  const attachmentsList = (items: unknown) => {
    ...
    const embedded = attachments
      .map((file) => embeddedAttachmentForExport(file))
      .join("");
    return `${table}${embedded}`;
  };
  ...
  const checklistExportHtml = (forcedChecklistNo?: number) => {
    const rawItems = normalizeChecklistItems(checklistForm.items) as Array<
      ChecklistItem & {
    ...
    const exportProjectName =
      (checklistForm as any).projectNameDisplay ||
      profile?.projectName ||
      projectName ||
      "";
    const exportContractor =
      checklistForm.contractor || profile?.contractor || "";
    ...
    const displayedItems = rawItems.filter(
      (item) => !Boolean((item as any).excludedFromPrint),
    );
  ...
  const uniqueEmailAttachments = (
    ...
    return attachments.filter(
      (attachment): attachment is OutgoingEmailAttachment => {
  ...
  const collectCurrentFormEmailAttachments = (): OutgoingEmailAttachment[] => {
    ...
    const walkForAttachments = (
      value: unknown,
      fallbackName = "מסמך מצורף",
    ) => {
      ...
      pushAttachment(
        name,
        obj.attachmentDataUrl,
        obj.attachmentType || mimeType,
      );
      ...
      pushAttachment(
        name,
        obj.contentBase64
          ? `data:${mimeType || "application/octet-stream"};base64,${obj.contentBase64}`
          : "",
        mimeType,
      );
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      if (existing) {
        ...
        existing.addEventListener(
          "error",
          () => reject(new Error(`טעינת ${label} נכשלה`)),
          { once: true },
        );
        if (test()) resolve();
        return;
  ...
  const collectCurrentFormPdfAppendices = () =>
  ...
  const buildFormOnlyPdfBytes = async (html: string, title: string) => {
    ...
    try {
      host
        .querySelectorAll(".attachment-page")
      ...
      if (!canvas.width || !canvas.height)
        throw new Error("יצירת צילום הטופס נכשלה");
      ...
      while (y < canvas.height) {
        ...
        pdf.addImage(
          ...
          margin,
          usableWidth,
          Math.min(imgHeightMm, usableHeight),
          undefined,
          "FAST",
        );
  ...
  const appendAttachmentToPdf = async (
    targetPdf: any,
    ...
    const bytesInfo = attachment.contentBase64
      ? {
          ...
          bytes: Uint8Array.from(
            atob(attachment.contentBase64.replace(/\s/g, "")),
            (c) => c.charCodeAt(0),
          ),
        }
      : src.startsWith("data:")
  ...
  const sendCurrentFormEmail = async () => {
    try {
      const activeProjectUsers = currentProjectEmailUsers.filter(
        (user) => user.active && isValidEmailAddress(user.email),
      );
      const usersText = activeProjectUsers
        .map(
          (user, index) =>
            `${index + 1}. ${user.name}${user.role ? " - " + user.role : ""}${user.company ? " - " + user.company : ""} <${user.email}>`,
        )
        .join("\n");
...
דוגמה: 1,3 או user@email.com, other@email.com`
      ...
      const recipientInput = window.prompt(
        promptText,
        activeProjectUsers.length ? "1" : FIXED_EMAIL_RECIPIENT,
      );
      ...
      const selectedEmails = rawRecipients.flatMap((value) => {
        ...
        if (
          ...
          index >= 1 &&
          index <= activeProjectUsers.length
        )
          return [activeProjectUsers[index - 1].email];
        return [value];
      });
      const uniqueRecipients = Array.from(
        new Set(selectedEmails.map((email) => email.trim()).filter(Boolean)),
      );
      ...
      <div style={styles.layout}>
        <main style={styles.mainCard}>
                ...
                columns={[
                  {
                    label: "מספר",
                    value: (record) =>
                      record.processNo || record.number || record.id,
                  },
                  { label: "כותרת", value: (record) => getRecordTitle(record) },
                  {
                    label: "תחום",
                    value: (record) =>
                      record.workType || record.category || record.type,
                  },
                  {
                    label: "מיקום / שימוש",
                    value: (record) => record.location || record.area,
                  },
                  {
                    label: "סטטוס",
                    value: (record) => getRecordStatus(record),
                  },
                ]}
                onOpen={(id) => {
                  const record = projectControlProcesses.find(
                    (item) => item.id === id,
                  );
                  if (record) loadControlProcess(record);
                }}
              ...
              <FolderRecordsTable
                ...
                columns={[
                  { label: "כותרת", value: (record) => getRecordTitle(record) },
                  {
                    label: "מספר",
                    value: (record) =>
                      record.rfiNo || record.number || record.id,
                  },
                ...
                onOpen={(id) => {
                  const record = projectRfis.find((item) => item.id === id);
                  if (record) loadRfi(record);
                }}
                onDelete={deleteRfi}
                onNew={resetRfiForm}
              />
          ...
          {section === "supervisionReports" && (
            <FolderRecordsTable
              title="דוחות פיקוח עליון"
              description="ניתן להוסיף דוח, לתת שם ולנהל סטטוס: מאושר או בטיפול."
              records={currentSupervisionReports as any[]}
              columns={[
                { label: "שם", value: (record) => record.name },
                { label: "סטטוס", value: (record) => record.status },
                { label: "תאריך", value: (record) => record.createdAt },
              ]}
              onOpen={(id) => updateSupervisionStatus(id)}
              onDelete={deleteSupervisionReport}
              onNew={addSupervisionReport}
            />
          )}
          {section === "home" && (
            <HomeSection
          ...
          {section === "checklists" && (
            <>
              ...
              <FolderRecordsTable
                ...
                columns={[
                  {
                    label: "מספר",
                    value: (record) =>
                      record.checklistNo || record.number || record.id,
                  },
                  { label: "כותרת", value: (record) => getRecordTitle(record) },
                  {
                    label: "קטגוריה",
                    value: (record) => record.category || record.templateKey,
                  },
                ...
                onOpen={(id) => {
                  const record = projectChecklists.find(
                    (item) => item.id === id,
                  );
                  if (record) loadChecklist(record);
                }}
          ...
          {section === "nonconformances" && (
            <>
              <FolderRecordsTable
                ...
                onOpen={(id) => {
                  const record = projectNonconformances.find(
                    (item) => item.id === id,
                  );
                  if (record) loadNonconformance(record);
                }}
              ...
              <FolderRecordsTable
                ...
                columns={[
                  { label: "כותרת", value: (record) => getRecordTitle(record) },
                  {
                    label: "מיקום",
                    value: (record) =>
                      record.location || record.roadStructure || record.area,
                  },
                  { label: "תאריך", value: (record) => getRecordDate(record) },
                  {
                    label: "סטטוס",
                ...
                onOpen={(id) => {
                  const record = projectTrialSections.find(
                    (item) => item.id === id,
                  );
          ...
          {section === "preliminary" && (
            <>
              <FolderRecordsTable
                ...
                records={
                  projectPreliminary.filter(
                    (record) => record.subtype === preliminaryTab,
                  ) as any[]
                }
                columns={
                  preliminaryTab === "subcontractors"
                    ? [
                        {
                          label: "שם קבלן מאושר",
                          value: (record) => getPreliminaryName(record),
                        },
                        {
                          label: "תחום עבודה",
                          value: (record) => getPreliminaryMainField(record),
                        },
                        {
                          label: "תאריך אישור",
                          value: (record) =>
                            getDocumentApprovalDate(record) ||
                            getRecordDate(record),
                        },
                        {
                          label: "תאריך תפוגה",
                          value: (record) => getDocumentExpiryDate(record),
                        },
                        {
                          label: "סטטוס",
                          value: (record) =>
                            formatExpiredStatus(getDocumentExpiryDate(record)),
                        },
                      ]
                    : preliminaryTab === "suppliers"
                      ? [
                      ...
                      : [
                          ...
                          {
                            label: "סוג חומר מסופק",
                            value: (record) =>
                              getPreliminarySecondaryField(record),
                          },
                          {
                            label: "תאריך תפוגה",
                            value: (record) => getDocumentExpiryDate(record),
                ...
                onOpen={(id) => {
                  const record = projectPreliminary.find(
                    (item) => item.id === id,
                  );
                  if (record) loadPreliminary(record);
                }}
