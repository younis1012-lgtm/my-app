    ...
    ? items.map(
        (item: any, index) =>
          ({
            ...
            signature: item?.signature ? normalizeProcessSignature(item.signature, item?.responsible || "גורם אחראי", item?.inspector || "") : undefined,
...
export default function Page() {
  ...
  const loadFromCloudResults = (
    ...
    setSavedChecklists(
      (checklistRows ?? []).map((row) => ({
        ...
        contractor: row.contractor ?? "",
        notes: row.notes ?? "",
        projectNameDisplay: getChecklistMetaFromApproval(row.approval).projectNameDisplay ?? "",
        roadStructure: getChecklistMetaFromApproval(row.approval).roadStructure ?? "",
        stationSection: getChecklistMetaFromApproval(row.approval).stationSection ?? "",
        toStationSection: getChecklistMetaFromApproval(row.approval).toStationSection ?? "",
        offset: getChecklistMetaFromApproval(row.approval).offset ?? "",
  ...
  const allocateNextChecklistNo = (projectId: string) => {
    const maxSaved = getMaxSavedChecklistNo(projectId);
  ...
  const getChecklistMetaFromForm = (form: any) => ({
    projectNameDisplay: String(form?.projectNameDisplay ?? ""),
    roadStructure: String(form?.roadStructure ?? ""),
    stationSection: String(form?.stationSection ?? ""),
    toStationSection: String(form?.toStationSection ?? ""),
    offset: String(form?.offset ?? ""),
  });

  const getChecklistMetaFromApproval = (approval: any) =>
    approval && typeof approval === "object" && approval.checklistMeta && typeof approval.checklistMeta === "object"
      ? approval.checklistMeta
      : {};

  const buildChecklistApprovalWithMeta = (approval: ApprovalFlow, form: any) => ({
    ...normalizeApproval(approval),
    checklistMeta: getChecklistMetaFromForm(form),
  });

  const ensureRoad806SurveyorSignatures = (items: any[]) =>
    normalizeChecklistItems(items).map((item: any) => {
      if (!isRoad806ProjectName(projectName) || !isSurveyorRoleValue(item.responsible)) return item;
      const autoName = resolveResponsibleName(item.responsible, projectName) || item.inspector || "באסל שקארה";
      const existing = normalizeProcessSignature(item.signature, item.responsible || "מודד", autoName);
      if (existing.signature) return { ...item, signature: existing };
      return {
        ...item,
        inspector: autoName,
        signature: {
          ...existing,
          role: item.responsible || "מודד",
          signerName: existing.signerName || autoName,
          signature: ROAD_806_SURVEYOR_STAMP_DATA_URL,
          signedAt: existing.signedAt || item.executionDate || new Date().toISOString().slice(0, 10),
        },
      };
    });
  ...
  const saveChecklist = async () => {
    ...
    const record: ChecklistRecord = {
      ...
      ...checklistForm,
      items: ensureRoad806SurveyorSignatures(checklistForm.items),
      approval: buildChecklistApprovalWithMeta(checklistForm.approval, checklistForm),
      savedAt: nowLocal(),
    };
    await withSaving(async () => {
      if (cloudEnabled) {
        ...
        setSavedChecklists((prev) =>
          editingChecklistId
            ? prev.map((item) => (item.id === editingChecklistId ? record : item))
            : [record, ...prev],
        );
        await refreshCloudData();
      } else {
  ...
  const loadChecklist = (record: ChecklistRecord) => {
    ...
    setChecklistForm({
      ...
      contractor: record.contractor,
      notes: record.notes,
      projectNameDisplay: (record as any).projectNameDisplay ?? getChecklistMetaFromApproval((record as any).approval).projectNameDisplay ?? "",
      roadStructure: (record as any).roadStructure ?? getChecklistMetaFromApproval((record as any).approval).roadStructure ?? "",
      stationSection: (record as any).stationSection ?? getChecklistMetaFromApproval((record as any).approval).stationSection ?? "",
      toStationSection: (record as any).toStationSection ?? getChecklistMetaFromApproval((record as any).approval).toStationSection ?? "",
      offset: (record as any).offset ?? getChecklistMetaFromApproval((record as any).approval).offset ?? "",
  ...
  const checklistExportHtml = (forcedChecklistNo?: number) => {
    const rawItems = ensureRoad806SurveyorSignatures(checklistForm.items) as Array<ChecklistItem & { attachments?: ChecklistAttachment[]; signature?: ProcessSignature; excludedFromPrint?: boolean }>;
