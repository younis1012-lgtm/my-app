...
export default function Page() {
  ...
  const exportLogoHtml = () => `<div class="export-logo-wrap"><img class="doc-logo" src="${CONTROLENG_LOGO_DATA_URI}" alt="CONTROLENG PRIME LTD" /></div>`;
  ...
  const exportStyles = `
    ...
    .export-logo-wrap{width:100%;text-align:left;margin:0 0 3px}
    .doc-logo{width:82px;height:auto;display:inline-block}
    ...
    .trial-report .trial-title{font-size:15px;font-weight:900}
    .trial-report .label{font-weight:800;width:32%}
    .trial-report .value{height:20px}
    .trial-report .large-value{height:48px}
    @page{size:A4 landscape;margin:5mm}
    @media print{button{display:none} body{padding:0;font-size:8px}.header-title{font-size:13px} th,td{padding:1px 2px}.doc-header td{height:15px}.source-meta td{height:14px}.check-table td{height:14px}.check-table th{height:14px}.doc-logo{width:78px}.trial-report{width:82%}}
  `;
  ...
  const trialSectionExportHtml = () => {
    ...
    // משאירים את הכותרות בלבד ומוחקים ערכי דוגמה מהטופס.
    ...
    <table class="trial-report">
      <tbody>
        <tr><th class="label">שם הפרויקט</th><td class="value">${trialBlank()}</td></tr>
        <tr><th class="label">חברת ניהול</th><td class="value">${trialBlank()}</td></tr>
        <tr><th class="label">קבלן ראשי</th><td class="value">${trialBlank()}</td></tr>
        <tr><th class="label">חברת בקרת איכות</th><td class="value">${trialBlank()}</td></tr>
      </tbody>
    </table>
    <table class="trial-report">
      <tbody>
  ...
  const exportHtml = (forcedChecklistNo?: number) => {
    ...
    const body = section === 'checklists' ? checklistExportHtml(forcedChecklistNo)
      ...
      : '';
    return `<!doctype html><html lang="he" dir="rtl"><head><meta charset="utf-8"><title>${safeText(title)}</title><style>${exportStyles}</style></head><body>${exportLogoHtml()}<h1>${safeText(title)}</h1><div class="meta">פרויקט: ${safeText(projectName)}</div>${body}</body></html>`;
  };
