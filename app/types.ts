...
export type ChecklistTemplateKey = 'general' | 'paintWorks' | 'milling' | 'rockWall' | 'excavation' | 'channelPaving' | 'baseCourseSpreading' | 'curbstones' | 'asphaltSite' | 'castCurbstone' | 'catsEyes' | 'siteConcrete' | 'jkWorks' | 'controlledCompaction' | 'standardCompaction' | 'guardrails' | 'signage' | 'waterSystems' | 'paving' | 'steelGuardrailsSupply' | 'asphaltWorks' | 'drainagePiping' | 'electricalControlCells' | 'electricalCrossingPipesCables' | 'electricalLightingPole';
...
export type ChecklistRecord = { id: string; projectId: string; checklistNo?: number; templateKey: ChecklistTemplateKey; title: string; category: string; location: string; date: string; contractor: string; notes: string; items: ChecklistItem[]; approval: ApprovalFlow; savedAt: string; controlProcessId?: string; specSection?: string; workType?: string; selectedPlanId?: string; executionPlanNo?: string; executionPlanName?: string; executionPlanRevision?: string; };
...
export type RequiredDocument = { id: string; type: 'תעודת מעבדה' | 'רשימת מדידה' | 'צילום' | 'אישור ספק' | 'תוכנית' | 'אחר'; required: boolean; attached: boolean; attachmentId?: string; description?: string; certificateNo?: string; expiryDate?: string; attachmentName?: string; attachmentDataUrl?: string; attachmentType?: string; };
