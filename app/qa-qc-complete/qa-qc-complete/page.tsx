...
const normalizeApproval = (value: unknown): ApprovalFlow => {
  ...
  return {
    ...
    signatures: base.signatures.map((entry) => {
      const found = signatures.find((s: any) => s?.role === entry.role) as any;
      return {
        ...entry,
...
const createDefaultNonconformance = (): Omit<NonconformanceRecord, 'id' | 'projectId' | 'savedAt'> => ({ title: '', location: '', date: '', raisedBy: '', severity: 'בינונית', status: 'פתוח', description: '', actionRequired: '', notes: '', approval: createDefaultApproval() });
const createDefaultTrialSection = (): Omit<TrialSectionRecord, 'id' | 'projectId' | 'savedAt'> => ({ title: '', location: '', date: '', spec: '', result: '', approvedBy: '', status: 'טיוטה', notes: '', approval: createDefaultApproval() });
...
export default function Page() {
  ...
  const saveNonconformance = async () => {
    ...
    await withSaving(async () => {
      if (cloudEnabled) {
        const payload = { id: record.id, project_id: record.projectId, title: record.title, location: record.location, date: record.date, raised_by: record.raisedBy, severity: record.severity, status: record.status, description: record.description, action_required: record.actionRequired, notes: record.notes, approval: record.approval, saved_at: nowIso() };
  ...
  const loadNonconformance = (record: NonconformanceRecord) => { setSection('nonconformances'); setEditingNonconformanceId(record.id); setNonconformanceForm({ title: record.title, location: record.location, date: record.date, raisedBy: record.raisedBy, severity: record.severity, status: record.status, description: record.description, actionRequired: record.actionRequired, notes: record.notes, approval: normalizeApproval(record.approval) }); };
  ...
  const saveTrialSection = async () => {
    ...
    await withSaving(async () => {
      if (cloudEnabled) {
        const payload = { id: record.id, project_id: record.projectId, title: record.title, location: record.location, date: record.date, spec: record.spec, result: record.result, approved_by: record.approvedBy, status: record.status, notes: record.notes, approval: record.approval, saved_at: nowIso() };
